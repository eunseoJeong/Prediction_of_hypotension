...
model <- keras_model_sequential() %>% 
  layer_dense(units = 122, activation = "relu", input_shape=c(61)) %>% 
...
model %>% fit(
  x_train,
  y_train,
  epochs = 50, # train 텐서의 모든 표본을 대상으로 삼아 20회 반복
)

history %>% str()

model %>% predict(x_test) %>% table()

# 1. 핸즈온 머신러닝 9장 anomaly detection --> 취소
# 2. gausian mixture 모델 적용해서 outlier 도출 사용 --> 취소
